...
To give energy consumption estimates, first it is necessary to obtain energy profiles so the inference function can obtain the most accurate results possible, for that, a process was created in order to efficiently obtain the energy usage of code snippets.
...
Using the process explained on \ref{sec:work1} it was noticeble that the Java orchestrator was getting significantly more energy consumption than the Python one, which is not very logical, since they both target the same program. So, to try and check which one was having problems, two more orchestrators were implemented, one in C and another in bash.
