...
To further evaluate the framework’s ability to analyze and collect energy models for various custom programs, three additional benchmarks from the same source were selected. The chosen programs were fannkuch-redux (Java), n-body (Java naot #4), and spectral-norm (Java). These benchmarks were specifically selected because they are purely CPU and memory bound, with minimal variability and no reliance on disk I/O or multithreading. This makes them more predictable and reliable for energy consumption measurements.

The predictions for the fannkuch-redux and spectral-norm programs are shown in Tables~\ref{tab:energy_comparison_fannkuch_redux} and~\ref{tab:energy_comparison_spectral_norm}, respectively. Overall, the results are quite reasonable, with the model achieving low prediction errors in most cases. However, for the n-body program, the model performed poorly, as seen in Table~\ref{tab:energy_comparison_nBody}, where the predicted values deviated by an order of magnitude. 

The Figures~\ref{fig:advance_plot},~\ref{fig:approximate_plot}, and~\ref{fig:fannkuch_plot}, show the energy readings of the methods of the different programs, and their predictions.
