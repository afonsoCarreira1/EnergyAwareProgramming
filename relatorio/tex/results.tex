...
{\color{blue}For the \texttt{nbody} benchmark, three methods are involved: \texttt{offsetMomentum(\allowbreak\ double,\allowbreak\ double,\allowbreak\ double)},
...
\begin{table}[htbp]
  ...
  \setlength{\tabcolsep}{10pt} 
  \makebox[\textwidth][c]{%
    \begin{tabular}{@{}p{2.5cm}@{\hspace{2.5em}}c@{\hspace{1em}}c@{\hspace{1em}}p{7cm}@{}}
      \toprule
      Method & Pearson & Spearman & Interpretation \\
      \toprule  

      \texttt{checkTree} & 0.83 & 0.90 & Strong, slightly nonlinear relationship. \\
      \texttt{trees} & 0.84 & 0.97 & Very strong nonlinear relationship. \\
      \texttt{createTree} & 0.54 & 0.97 & Moderate linear relationship, but strong nonlinear. \\
      \texttt{advance} & -0.05 & -0.03 & No meaningful correlation. \\
      \texttt{Approximate} & 0.97 & 0.99 & Extremely strong, nearly perfect correlation. \\
      \texttt{fannkuch} & 0.52 & 0.97 & Moderate linear correlation, but strong nonlinear. \\
...
In its current form, the frameworks most important feature collected from the benchmarks is the input, and it showed to be effective in helping the models outputting a reliable prediction model. However, it is possible to see that it is the only variable that can help the models predict in this case. They could benefit from a more diverse set of features that could include dynamic features, for instance, time of execution, processor cycles or other low-level metrics. Using more features could help the overall accuracy of the models.
